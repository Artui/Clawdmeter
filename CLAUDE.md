...
firmware/src/
  ...
  buddy.{h,cpp}             — "Claude Buddy" companion screen. Renders ASCII art (Mono font) + name/level/★ + 5 stat bars from BuddyState, a transient speech bubble (creature tucks below it while talking), and a rarity-color tint (RARITY_COLOR[], upstream palette w/ brightened common) applied to stars/name/creature per the `tint` bitmask. Device is dumb; daemon/buddy.py is the brain.
...
**Navigation.** Top-level screens cycle **Data ↔ Splash ↔ Buddy** via `ui_screen_next/prev` (Data == last Usage/BT screen, tracked in `prev_data_screen`). "Within" a screen = `ui_cycle_within`: Usage↔Bluetooth / `splash_next` / `buddy_cycle_view` (buddy sub-views Full→Creature→Stats). Touch = `ui_screen_next`. **Buttons are configurable** (`controls.{h,cpp}`, NVS): each slot (primary/secondary/pwr-short/pwr-long) → an action (screen-next/prev, cycle-within, sleep, none); default = board-aware navigation. `main.cpp::dispatch_action` runs them; PWR long-press → `idle_sleep_now()`. **LEFT+RIGHT combo** (two-button boards only): holding both fires a random device-local buddy phrase (`buddy_say_random`) and jumps to the buddy screen. To let the combo claim the press, single primary/secondary actions fire on **release** (not press) on two-button boards; the combo is latched per hold-cycle and suppressed if the press woke the device. Single-button boards keep press-edge firing.
...
**Buddy brain (`daemon/buddy.py`).** Imported by the Python daemon; each poll appends a compact `b` block to the payload. Derives a deterministic buddy (wyhash→mulberry32→bones, ported from MIT upstream) from an identity resolved as: config overrides → `seed`/`$BUDDY_SEED` → cached account id (JWT `sub`, else host fallback). Config: `~/.config/claude-usage-monitor/buddy.toml` (see `daemon/buddy.example.toml`); state/XP: `buddy.json`.

**Buddy remarks (the buddy "talks").** Optional `rm` (text) + `rf` (force-foreground) fields on the `b` block drive a transient speech bubble on the device (`buddy.cpp` renders it; `BuddyState.remark`; `ui.cpp` does the configurable foreground pop). `compute_remark()` picks an ASCII-only line — curated tables keyed by mood/usage, plus reactions to real coding events — gated by `[remarks]` config (`enabled`/`cooldown_secs`/`chattiness`/`foreground`/`llm_rate`/`model`, all hot-reloaded each poll). `rm` is attached only when there's something *new* to say; the firmware re-pops only on a changed string. Lines must stay ASCII (device fonts) and ≤~80 chars. Bubble lifetime is configurable: `[remarks] bubble_secs` → `bd` on the block → `buddy_set_bubble_ms` (default 7 s; also used by the LEFT+RIGHT combo and synced to the foreground-pop window via `buddy_get_bubble_ms`). The device-local combo phrase pool is `BUDDY_PHRASES[]` in `buddy.cpp`.

**Hooks → spool (Phase 2, wired).** `daemon/buddy_hook.py` is a zero-dependency Claude Code hook (registered in `~/.claude/settings.json` on SessionStart/UserPromptSubmit/PostToolUse[Edit|Write|MultiEdit|NotebookEdit|Bash]/Stop/SessionEnd). It appends one compact event line per hook to `~/.config/claude-usage-monitor/buddy-events.jsonl`. `buddy.py::drain_events()` rename-then-reads (atomic consume) the spool each poll; `summarize_events()` folds it into an activity summary that feeds **mood** (errors→chaotic, edit streak→focused, busy→excited), **XP** (real edits/bash/prompts earn the bulk), and **event remarks** (notable events bypass the chattiness gate). No events → ambient time-of-day + usage-pressure mood. The hook path is absolute — repoint it in settings.json if the repo moves. `make buddy-*` manage state.

**Rarity colors.** The `b` block carries a `tc` tint bitmask (1=stars, 2=name, 4=creature) from `buddy.toml`'s `[appearance]` section (default stars+creature, hot-reloaded). The fixed rarity palette lives device-side in `buddy.cpp::RARITY_COLOR[]` (ported from upstream `server/theme.ts` dark theme — common gray brightened to `0xBBBBBB` for the black AMOLED); `apply_rarity_colors()` tints the enabled labels, others keep theme defaults.

**LLM remarks (Phase 3, wired).** When the `llm_rate` gate fires, `buddy.py::_maybe_llm_req` returns a prompt (situation in the user turn; static persona in `buddy.LLM_PERSONA`, sent after `buddy.LLM_IDENTITY` — the standalone first system block the Claude Code OAuth token requires). The daemon's `generate_llm_remark()` POSTs it to `/v1/messages` with the same OAuth token/headers as the usage poll (system block sent `cache_control:ephemeral`), then overwrites `block["rm"]` on success — the curated line is the automatic fallback. Spends the user's quota, so default `llm_rate=0`.
