...
static void start_advertising() {
    ...
    adv->reset();
    // Primary advertising packet (≤31 bytes): flags + name "Claude Controller".
    // The device is no longer a HID keyboard, so it intentionally does NOT
    // advertise the 0x1812 HID service and won't appear in the macOS Bluetooth
    // Settings GUI. That's fine — the host daemon discovers it by name via an
    // active scan and reads the 128-bit custom service UUID from the scan
    // response below; no OS pairing is involved.
...
void ble_init(void) {
    ...
    // --- Custom data service (the only service; no HID anymore) ---
